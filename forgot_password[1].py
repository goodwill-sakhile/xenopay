...
class ForgotPasswordScreen(MDScreen):
    #Screen to load when you want to recover password
    ...
    def makeRequest(self, seconds):
        try:
            ...
            if response[0]  == 1:
                ...
                self.response = 0
                self.error_message = "Could'nt recognise the email"
        except:
            self.response = -2
...
class ChangePasswordButton(TouchBox):
    def respondToTouch(self):
        ...
        if not (code and n_password and con_password):
            self.callDialog(text = "Fill in all text input")
    def callDialog(self, text):
        button = MDFillRoundFlatButton(text = "Ok")
