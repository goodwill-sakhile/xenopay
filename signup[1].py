...
class SignUpBodyPageScreen(MDScreen):
    ...
    def makeRequest(self, seconds):
        try:
            print("Make signup requests")
            response = eval(requests.get("http://localhost:8080/gateway/signup/", params = self.params).text)
            print("Signup Requests made")
            if response[0]  == 1:
                self.id = response[1]
                self.response = 1
            else:
                self.response = 0
                self.error_message = "Could'nt signup existing user"
        except:
            self.response = -2
...
class SignUpFlatButton(TouchBox):
    ...
    def respondToTouch(self):
        ...
        if not text_fields_check:
            ...
            if self.root.touch:
                self.root.params = {"full_names":names, "last_name":last_name, "email":email, "password":password_one}
